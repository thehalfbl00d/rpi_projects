...
class ContainerVersionBehavior:
  ...
  def create(self):
    ...
    click.echo(f'Got container: {container}')
    container_id = container.get('id')
  ...
  def upload_version_json(self, version_data, container_id):
    click.echo(f'VERSION DATA: {version_data}')
  ...
  def download_app_zip(self):
    ...
    try:
      ...
      raise click.ClickException(f'Container not found: {str(e)}')
  ...
  def update_variables(self):
    ...
    post_data = self.env
    resp = self.util.cli_request('POST',\
      self.util.build_url('{experienceCloudUri}/iot/v1/containers/{container_id}/versions/{version_id}/update_variables',
      {'container_id': container_id, 'version_id': version_id}), data=json.dumps(post_data))
